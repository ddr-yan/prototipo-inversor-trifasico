clear; clc; close all;
syms t tau ie(t) Rgloff Rglon Vplt Ciss Vf Vdrv Vbe2 B1 B2 Rc Rb ie2 Qgd t_on tsw Vbe3 Vb


% ---- Definir ecuación de carga del capacitor

eqn = -(Vbe3+Vb)+(1/Ciss)*int(ie(tau), tau, 0, t)== - ie(t)*(Rc/(B2+1)+Rglon) + Vdrv-Vbe2;
...
sol_num2 = subs(valorRglon, ...
    [B2,      tsw,      Ciss,     Rc,     Vplt,     Vbe2,     Vdrv,     Qgd    , Vb, Vbe3], ...
    [B2_val,  tsw_val,  Ciss_val, Rc_val, Vplt_val, Vbe2_val, Vdrv_val  Qgd_val, Vb_val,Vbe3_val]);
...
Rglon_val = 47;
sol_num1 = subs(ton, ...
    [B2,        Ciss,     Rc,     Vplt,     Vbe2,     Vdrv,     Qgd,     Rglon,   Vb,     Vbe3], ...
    [B2_val,    Ciss_val, Rc_val, Vplt_val, Vbe2_val, Vdrv_val  Qgd_val, Rglon_val, Vb_val, Vbe3_val]);

ton_val=double (sol_num1);

sol_num1 = subs(tpo, ...
    [B2,       Ciss,     Rc,     Vplt,     Vbe2,     Vdrv,        Rglon,   Vb,     Vbe3], ...
    [B2_val,   Ciss_val, Rc_val, Vplt_val, Vbe2_val, Vdrv_val, Rglon_val, Vb_val, Vbe3_val]);

tpo_val=double (sol_num1);

t_total=ton_val+tpo_val;
